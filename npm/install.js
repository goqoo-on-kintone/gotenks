...
// GitHub Releases から gotenks バイナリをダウンロードする postinstall スクリプト
...
const REPO = "goqoo-on-kintone/gotenks";
const BINARY_NAME = "gotenks";
...
function extractTarGz(buffer, destDir) {
  const tempFile = path.join(os.tmpdir(), `gotenks-${Date.now()}.tar.gz`);
...
function extractZip(buffer, destDir) {
  const tempFile = path.join(os.tmpdir(), `gotenks-${Date.now()}.zip`);
  fs.writeFileSync(tempFile, buffer);
...
async function main() {
  ...
  console.log(`Downloading gotenks v${VERSION} for ${platform}/${arch}...`);
  ...
  try {
    ...
    console.log("gotenks installed successfully!");
  } catch (error) {
    console.error(`Failed to install gotenks: ${error.message}`);
